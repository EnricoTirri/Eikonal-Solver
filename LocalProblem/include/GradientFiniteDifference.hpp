 ...
 */
    template<size_t PRDIM, FiniteDifferenceType FDT = FiniteDifferenceType::Centered>
    class GradientFiniteDifference {
    public:
        using Scalar = LineSearch::Scalar;
        using Vector = LineSearch::Traits<PRDIM>::Vector;

        GradientFiniteDifference() = default;

        //! Here you pass the function to be derived.
        GradientFiniteDifference(const LineSearch::Traits<PRDIM>::CostFunction &f) : f(f) {};

        //! Here you change the function to be derived.
        void
        setFunction(const LineSearch::Traits<PRDIM>::CostFunction &ff) {
            f = ff;
        }
    ...
    private:
        LineSearch::Traits<PRDIM>::CostFunction f;
        //! A small number used for differencing
        double const smallNumber = std::sqrt(std::numeric_limits<double>::epsilon());
    };

    template<size_t PRDIM, FiniteDifferenceType FDT>
    typename GradientFiniteDifference<PRDIM, FDT>::Vector
    GradientFiniteDifference<PRDIM, FDT>::operator()(const Vector &x) {
        auto n = x.size();
        Vector res(n);
