...
loadDirections()
{
  ...
  theFactory.add("GradientDirection",
                 []() { return std::make_unique<GradientDirection>(); });
  theFactory.add("BFGSDirection",
                 []() { return std::make_unique<BFGSDirection>(); });
  theFactory.add("BFGSIDirection",
                 []() { return std::make_unique<BFGSIDirection>(); });
  theFactory.add("BBDirection",
                 []() { return std::make_unique<BBDirection>(); });
  theFactory.add("CGDirection",
                 []() { return std::make_unique<CGDirection>(); });
  theFactory.add("NewtonDirection",
                 []() { return std::make_unique<NewtonDirection>(); });
  return theFactory;
}
} // namespace apsc
