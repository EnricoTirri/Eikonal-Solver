...
$$\begin{cases}
H(\bm{x}, \nabla u(\bm{x})) = 1 & \quad \bm{x} \in \Omega \subset \mathbb{R}^d \\  
u(\bm{x}) = g(\bm{x}) & \quad \bm{x} \in \Gamma \subset \partial\Omega
\end{cases} $$
...
$$
\begin{cases}
\left|\nabla u(\bm{x})\right| = 1/c & \quad \bm{x} \in \Omega\\
u(\bm{x}) = g(\bm{x}) & \quad \bm{x}\in \Gamma \subset \partial\Omega
\end{cases}
$$
